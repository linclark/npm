...
function install (args, cb_) {
  ...
  function cb (er, installed) {
    ...
    findPeerInvalid(where, function (er, problem) {
      ...
      if (PATH.indexOf(npm.config.get("prefix") + "/bin") === -1) {
        npm.config.get("logstream").write("\nThe path that npm is using for global installations is not in your system's PATH env variable.\n")
      }
    })
  }
